...
progress_data: dict[int, tuple[float, int]] = {}  # {chat_id: (last_percent, message_id)}
...
async def handle_speed(cb: types.CallbackQuery):
    ...
    try:
        ...
        await cb.message.answer(
            f"Задача на скорость {speed}× принята. До тебя в очереди {task_queue.qsize()-1} видео."
        )
...
async def task_worker():
    while True:
        ...
        try:
            ...
            await bot.send_message(chat_id, "Ошибка: не удалось загрузить видео с YouTube.")
            logger.error(f"Ошибка загрузки: {url}, {e}")
        except subprocess.CalledProcessError as e:
            await bot.send_message(chat_id, "Ошибка: не удалось обработать аудио.")
            logger.error(f"Ошибка обработки: {url}, {e}")
        except Exception as e:
            await bot.send_message(chat_id, f"Неизвестная ошибка: {e}")
